...
class _ProductCardState extends State<ProductCard> {
  ...
  Widget _buildImageSection(GroceryProduct p) {
    ...
    Widget imageWidget;
    if (hasNetworkImage && !_imageError) {
      imageWidget = Image.network(
        ...
        errorBuilder: (_, _, _) {
          _imageError = true;
          return Center(child: Text(p.emoji, style: const TextStyle(fontSize: 28)));
        },
      );
    } else {
      imageWidget = Center(child: Text(p.emoji, style: const TextStyle(fontSize: 28)));
    }
